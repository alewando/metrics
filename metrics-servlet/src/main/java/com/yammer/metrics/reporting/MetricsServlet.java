...
/**
 * An HTTP servlet which outputs the metrics in a {@link MetricsRegistry} (and
 * optionally the data provided by {@link VirtualMachineMetrics}) in a JSON
 * object. Only responds to {@code GET} requests.
 * <p/>
 * If the servlet context has an attribute named
 * {@code com.yammer.metrics.reporting.MetricsServlet.registry} which is a
 * {@link MetricsRegistry} instance, {@link MetricsServlet} will use it instead
 * of {@link Metrics}.
 * <p/>
 * {@link MetricsServlet} also takes an initialization parameter,
 * {@code show-jvm-metrics}, which should be a boolean value (e.g.,
 * {@code "true"} or {@code "false"}). It determines whether or not JVM-level
 * metrics will be included in the JSON output.
 * <p/>
 * {@code GET} requests to {@link MetricsServlet} can make use of the following
 * query-string parameters:
 * <dl>
 * <dt><code>/metrics?class=com.example.service</code></dt>
 * <dd>
 * <code>class</code> is a string used to filter the metrics in the JSON by
 * metric name. In the given example, only metrics for classes whose canonical
 * name starts with <code>com.example.service</code> would be shown. You can
 * also use <code>jvm</code> for just the JVM-level metrics.</dd>
 * 
 * <dt><code>/metrics?pretty=true</code></dt>
 * <dd>
 * <code>pretty</code> determines whether or not the JSON which is returned is
 * printed with indented whitespace or not. If you're looking at the JSON in the
 * browser, use this.</dd>
 * 
 * <dt><code>/metrics?full-samples=true</code></dt>
 * <dd>
 * <code>full-samples</code> determines whether or not the JSON which is
 * returned will include the full content of histograms' and timers' reservoir
 * samples. If you're aggregating across hosts, you may want to do this to allow
 * for more accurate quantile calculations.</dd>
 * </dl>
 */
public class MetricsServlet extends HttpServlet implements
		MetricProcessor<MetricsServlet.Context> {

	/**
	 * The attribute name of the {@link MetricsRegistry} instance in the servlet
	 * context.
	 */
	public static final String REGISTRY_ATTRIBUTE = MetricsServlet.class
			.getName() + ".registry";

	/**
	 * The initialization parameter name which determines whether or not
	 * JVM_level metrics will be included in the JSON output.
	 */
	...
	public MetricsServlet() {
		this(Clock.defaultClock(), VirtualMachineMetrics.getInstance(), Metrics
				.defaultRegistry(), DEFAULT_JSON_FACTORY, true);
	}

	/**
	 * Creates a new {@link MetricsServlet}.
	 * 
	 * @param showJvmMetrics
	 *            whether or not JVM-level metrics will be included in the
	 *            output
	 */
	public MetricsServlet(boolean showJvmMetrics) {
		this(Clock.defaultClock(), VirtualMachineMetrics.getInstance(), Metrics
				.defaultRegistry(), DEFAULT_JSON_FACTORY, showJvmMetrics);
	}
	 ...
	 * 
	 * @param clock
	 *            the clock used for the current time
	 * @param vm
	 ...
	 *            output
	 */
	public MetricsServlet(Clock clock, VirtualMachineMetrics vm,
			MetricsRegistry registry, JsonFactory factory,
	...
	public void init(ServletConfig config) throws ServletException {
		final Object factory = config.getServletContext().getAttribute(
				JsonFactory.class.getCanonicalName());
		...
		final String showJvmMetricsParam = config
				.getInitParameter(SHOW_JVM_METRICS);
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		final boolean showFullSamples = Boolean.parseBoolean(req
				.getParameter("full-samples"));

		resp.setStatus(HttpServletResponse.SC_OK);
		resp.setContentType(CONTENT_TYPE);
		final OutputStream output = resp.getOutputStream();
		final JsonGenerator json = factory.createJsonGenerator(output,
				JsonEncoding.UTF8);
		if (pretty) {
		...
		{
			if (showJvmMetrics
					&& ("jvm".equals(classPrefix) || classPrefix == null)) {
				writeVmMetrics(json);
			}
	...
	private void writeVmMetrics(JsonGenerator json) throws IOException {
		...
		json.writeStartObject();
		{
			...
			json.writeStartObject();
			{
				...
				json.writeStartObject();
				{
					for (Map.Entry<String, Double> pool : vm.memoryPoolUsage()
							.entrySet()) {
			...
			json.writeStartObject();
			{
				for (Map.Entry<Thread.State, Double> entry : vm
						.threadStatePercentages().entrySet()) {
					json.writeNumberField(entry.getKey().toString()
			...
			json.writeStartObject();
			{
				for (Map.Entry<String, VirtualMachineMetrics.GarbageCollectorStats> entry : vm
					...
					json.writeStartObject();
					{
						...
						json.writeNumberField("time",
								gc.getTime(TimeUnit.MILLISECONDS));
					}
					json.writeEndObject();
				}
			}
	...
	public void writeRegularMetrics(JsonGenerator json, String classPrefix,
			boolean showFullSamples) throws IOException {
		for (Map.Entry<String, SortedMap<MetricName, Metric>> entry : registry
				.groupedMetrics().entrySet()) {
			if (classPrefix == null || entry.getKey().startsWith(classPrefix)) {
				json.writeFieldName(entry.getKey());
				json.writeStartObject();
				{
					for (Map.Entry<MetricName, Metric> subEntry : entry
							.getValue().entrySet()) {
						json.writeFieldName(subEntry.getKey().getName());
						try {
							subEntry.getValue().processWith(this,
									subEntry.getKey(),
									new Context(json, showFullSamples));
						} catch (Exception e) {
							LOGGER.warn(
									"Error writing out " + subEntry.getKey(), e);
						}
					}
				}
				json.writeEndObject();
	...
	public void processHistogram(MetricName name, Histogram histogram,
		...
		json.writeStartObject();
		{
			...
			if (context.showFullSamples) {
				json.writeObjectField("values", histogram.getSnapshot()
						.getValues());
			}
		}
		json.writeEndObject();
	...
	public void processTimer(MetricName name, Timer timer, Context context)
		...
		json.writeStartObject();
		{
			...
			json.writeStartObject();
			{
				json.writeStringField("unit", timer.durationUnit().toString()
						.toLowerCase());
				writeSummarizable(timer, json);
				writeSampling(timer, json);
				if (context.showFullSamples) {
					json.writeObjectField("values", timer.getSnapshot()
							.getValues());
				}
			}
			json.writeEndObject();
	...
	private static void writeSummarizable(Summarizable metric,
	...
	private static void writeSampling(Sampling metric, JsonGenerator json)
			throws IOException {
	...
	private static void writeMeteredFields(Metered metered, JsonGenerator json)
			throws IOException {
		json.writeStringField("unit", metered.rateUnit().toString()
				.toLowerCase());
