...
public class AdminServlet extends HttpServlet {
	...
	public AdminServlet(HealthCheckServlet healthCheckServlet,
			MetricsServlet metricsServlet, PingServlet pingServlet,
			String healthcheckUri, String metricsUri, String pingUri,
			String threadsUri) {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		this.metricsUri = getParam(config.getInitParameter("metrics-uri"),
				this.metricsUri);
		this.pingUri = getParam(config.getInitParameter("ping-uri"),
				this.pingUri);
		this.threadsUri = getParam(config.getInitParameter("threads-uri"),
				this.threadsUri);
		this.healthcheckUri = getParam(
				config.getInitParameter("healthcheck-uri"), this.healthcheckUri);
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (uri == null || uri.equals("/")) {
			...
			try {
				writer.println(MessageFormat.format(TEMPLATE, path, metricsUri,
						path, pingUri, path, threadsUri, path, healthcheckUri));
			} finally {
				writer.close();
			}
