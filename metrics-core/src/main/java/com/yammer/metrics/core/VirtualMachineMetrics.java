...
public class VirtualMachineMetrics {
	...
	public Map<String, Double> memoryPoolUsage() {
		...
		for (MemoryPoolMXBean pool : memoryPools) {
			final double max = pool.getUsage().getMax() == -1 ? pool.getUsage()
					.getCommitted() : pool.getUsage().getMax();
	 ...
	 * @see <a href="http://java.sun.com/j2se/versioning_naming.html">J2SE
	...
	/**
	 * Returns a map of garbage collector names to garbage collector
	 * information.
	 * 
	...
	// for (ThreadInfo info : this.threads.getThreadInfo(threadIds,
	// MAX_STACK_TRACE_DEPTH)) {
	// final StringBuilder stackTrace = new StringBuilder();
	// for (StackTraceElement element : info.getStackTrace()) {
	// stackTrace.append("\t at ").append(element.toString())
	// .append('\n');
	// }
	//
	// threads.add(String.format("%s locked on %s (owned by %s):\n%s",
	// info.getThreadName(), info.getLockName(),
	// info.getLockOwnerName(), stackTrace.toString()));
	// }
