...
public class Timer implements Metered, Stoppable, Sampling, Summarizable {
	 ...
	 * 
	 * @param tickThread
	 *            background thread for updating the rates
	 * @param durationUnit
	 *            the scale unit for this timer's duration metrics
	 * @param rateUnit
	 *            the scale unit for this timer's rate metrics
	 */
	Timer(ScheduledExecutorService tickThread, TimeUnit durationUnit,
			TimeUnit rateUnit) {
	 ...
	 * 
	 * @param event
	 *            a {@link Callable} whose {@link Callable#call()} method
	...
	/**
	 * Returns a timing {@link TimerContext}, which measures an elapsed time in
	 * nanoseconds.
	 * 
