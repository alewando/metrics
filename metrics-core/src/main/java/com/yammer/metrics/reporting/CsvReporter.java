...
/**
 * A reporter which periodically appends data from each metric to a
 * metric-specific CSV file in an output directory.
 */
public class CsvReporter extends AbstractPollingReporter implements
		MetricProcessor<CsvReporter.Context> {

	/**
	 * Enables the CSV reporter for the default metrics registry, and causes it
	 * to write to files in {@code outputDir} with the specified period.
	 * 
	 * @param outputDir
	...
	/**
	 * Enables the CSV reporter for the given metrics registry, and causes it to
	 * write to files in {@code outputDir} with the specified period.
	 * 
	 ...
	 *            the time unit of {@code period}
	 */
	public static void enable(MetricsRegistry metricsRegistry, File outputDir,
	...
	public interface Context {
		/**
		 * Returns an open {@link PrintStream} for the metric with
		 * {@code header} already written to it.
		 * 
		 * @param header
		 *            the CSV header
	...
	/**
	 * Creates a new {@link CsvReporter} which will write all metrics from the
	 * given {@link MetricsRegistry} to CSV files in the given output directory.
	 * 
	 * @param outputDir
	 *            the directory to which files will be written
	 * @param metricsRegistry
	 *            the {@link MetricsRegistry} containing the metrics this
	 ...
	 * {@link MetricsRegistry} which match the given {@link MetricPredicate} to
	 * CSV files in the given output directory.
	 * 
	 * @param metricsRegistry
	 *            the {@link MetricsRegistry} containing the metrics this
	 *            reporter will report
	 * @param predicate
	 *            the {@link MetricPredicate} which metrics are required to
	 *            match before being written to files
	 * @param outputDir
	 *            the directory to which files will be written
	 */
	public CsvReporter(MetricsRegistry metricsRegistry,
			MetricPredicate predicate, File outputDir) {
	 ...
	 */
	public CsvReporter(MetricsRegistry metricsRegistry,
			MetricPredicate predicate, File outputDir, Clock clock) {
	...
	/**
	 * Returns an opened {@link PrintStream} for the given {@link MetricName}
	 * which outputs data to a metric-specific {@code .csv} file in the output
	 * directory.
	 * 
	 * @param metricName
	...
	@Override
	public void run() {
		final long time = TimeUnit.MILLISECONDS.toSeconds(clock.time()
				- startTime);
		final Set<Entry<MetricName, Metric>> metrics = getMetricsRegistry()
				.allMetrics().entrySet();
		try {
			for (Entry<MetricName, Metric> entry : metrics) {
				...
				if (predicate.matches(metricName, metric)) {
					final Context context = new Context() {
						// @Override
						public PrintStream getStream(String header)
								throws IOException {
							final PrintStream stream = getPrintStream(
									metricName, header);
							stream.print(time);
							stream.print(',');
							return stream;
	...
	public void processMeter(MetricName name, Metered meter, Context context)
		...
		final PrintStream stream = context
				.getStream("# time,count,1 min rate,mean rate,5 min rate,15 min rate");
		stream.append(
				new StringBuilder().append(meter.count()).append(',')
	...
	public void processHistogram(MetricName name, Histogram histogram,
		...
		final PrintStream stream = context
				.getStream("# time,min,max,mean,median,stddev,95%,99%,99.9%");
		final Snapshot snapshot = histogram.getSnapshot();
		stream.append(
				new StringBuilder().append(histogram.min()).append(',')
	...
	public void processTimer(MetricName name, Timer timer, Context context)
		...
		stream.append(
				new StringBuilder().append(timer.min()).append(',')
						.append(timer.max()).append(',').append(timer.mean())
						.append(',').append(snapshot.getMedian()).append(',')
