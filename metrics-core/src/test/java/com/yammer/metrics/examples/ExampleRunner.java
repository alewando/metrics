...
public class ExampleRunner {
	...
	private static final Counter QUEUE_DEPTH = Metrics.newCounter(
			ExampleRunner.class, "queue-depth");
	private static final Histogram DIRECTORY_SIZE = Metrics.newHistogram(
			ExampleRunner.class, "directory-size", false);
	...
	public static class Job implements Runnable {
		...
		public void run() {
			try {
				while (true) {
					final File file = JOBS.poll(60, TimeUnit.SECONDS);
					QUEUE_DEPTH.dec();
					if (file.isDirectory()) {
						final List<File> contents = new DirectoryLister(file)
								.list();
