...
public class TimerTest {
	...
	public void hasADurationUnit() throws Exception {
		assertThat("the timer has a duration unit", timer.durationUnit(),
				is(TimeUnit.MILLISECONDS));
	}

	@Test
	public void hasARateUnit() throws Exception {
		assertThat("the timer has a rate unit", timer.rateUnit(),
				is(TimeUnit.SECONDS));
	}
	...
	public void aBlankTimer() throws Exception {
		...
		assertThat("the timer has a max duration of zero", timer.max(),
				is(closeTo(0.0, 0.001)));

		assertThat("the timer has a min duration of zero", timer.min(),
				is(closeTo(0.0, 0.001)));

		assertThat("the timer has a mean duration of zero", timer.mean(),
				is(closeTo(0.0, 0.001)));

		assertThat("the timer has a duration standard deviation of zero",
				timer.stdDev(), is(closeTo(0.0, 0.001)));

		final Snapshot snapshot = timer.getSnapshot();

		assertThat("the timer has a median duration of zero",
				snapshot.getMedian(), is(closeTo(0.0, 0.001)));

		assertThat("the timer has a 75th percentile duration of zero",
				snapshot.get75thPercentile(), is(closeTo(0.0, 0.001)));

		assertThat("the timer has a 99th percentile duration of zero",
				snapshot.get99thPercentile(), is(closeTo(0.0, 0.001)));

		assertThat("the timer has a mean rate of zero", timer.meanRate(),
				is(closeTo(0.0, 0.001)));

		assertThat("the timer has a one-minute rate of zero",
				timer.oneMinuteRate(), is(closeTo(0.0, 0.001)));

		assertThat("the timer has a five-minute rate of zero",
				timer.fiveMinuteRate(), is(closeTo(0.0, 0.001)));

		assertThat("the timer has a fifteen-minute rate of zero",
				timer.fifteenMinuteRate(), is(closeTo(0.0, 0.001)));

		assertThat("the timer has no values", timer.getSnapshot().size(), is(0));
	}
	...
	public void timingASeriesOfEvents() throws Exception {
		...
		assertThat("the timer has a max duration of 40", timer.max(),
				is(closeTo(40.0, 0.001)));

		assertThat("the timer has a min duration of 10", timer.min(),
				is(closeTo(10.0, 0.001)));

		assertThat("the timer has a mean duration of 24", timer.mean(),
				is(closeTo(24.0, 0.001)));

		assertThat("the timer has a duration standard deviation of zero",
				timer.stdDev(), is(closeTo(11.401, 0.001)));

		final Snapshot snapshot = timer.getSnapshot();

		assertThat("the timer has a median duration of 20",
				snapshot.getMedian(), is(closeTo(20.0, 0.001)));

		assertThat("the timer has a 75th percentile duration of 35",
				snapshot.get75thPercentile(), is(closeTo(35.0, 0.001)));

		assertThat("the timer has a 99th percentile duration of 40",
				snapshot.get99thPercentile(), is(closeTo(40.0, 0.001)));

		assertThat("the timer has no values", timer.getSnapshot().getValues(),
	...
	public void timingVariantValues() throws Exception {
		...
		assertThat("the timer has an accurate standard deviation",
				timer.stdDev(), is(closeTo(6.521908912666392E12, 0.001)));
	}
	...
	public void timingCallableInstances() throws Exception {
		...
		assertThat("returns the result of the callable", value, is("one"));

		assertThat("records the duration of the Callable#call()", timer.max(),
				is(closeTo(50, 500)));
	}
	...
	public void timingContexts() throws Exception {
		...
		assertThat("records the duration of the context", timer.max(),
				is(closeTo(50, 500)));
	}
}
