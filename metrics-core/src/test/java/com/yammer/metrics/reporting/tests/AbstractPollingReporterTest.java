...
public abstract class AbstractPollingReporterTest {
	...
	protected final <T extends Metric> void assertReporterOutput(
			Callable<T> action, String... expected) throws Exception {
		// Invoke the callable to trigger (ie, mark()/inc()/etc) and return the
		// metric
		final T metric = action.call();
		try {
			// Add the metric to the registry, run the reporter and flush the
			// result
			registry.add(new MetricName(Object.class, "metric"), metric);
			reporter.run();
			out.flush();
			final String[] lines = out.toString().split("\r?\n|\r");
			// Assertions: first check that the line count matches then compare
	...
	protected abstract AbstractPollingReporter createReporter(
			MetricsRegistry registry, OutputStream out, Clock clock)
			throws Exception;

	@Test
	public final void counter() throws Exception {
		final long count = new Random().nextInt(Integer.MAX_VALUE);
		assertReporterOutput(new Callable<Counter>() {
			// @Override
	...
	public final void histogram() throws Exception {
		assertReporterOutput(new Callable<Histogram>() {
			// @Override
	...
	public final void meter() throws Exception {
		assertReporterOutput(new Callable<Meter>() {
			// @Override
	...
	public final void timer() throws Exception {
		assertReporterOutput(new Callable<Timer>() {
			// @Override
	...
	public final void gauge() throws Exception {
		...
		assertReporterOutput(new Callable<Gauge<String>>() {
			// @Override
	...
	static Counter createCounter(long count) throws Exception {
		...
		return configureMatcher(mock, doAnswer(new MetricsProcessorAction() {
			@Override
			void delegateToProcessor(MetricProcessor<Object> processor,
					MetricName name, Object context) throws Exception {
	...
	@SuppressWarnings("unchecked")
	static <T extends Metric> T configureMatcher(T mock, Stubber stub)
			throws Exception {
		stub.when(mock).processWith(any(MetricProcessor.class),
				any(MetricName.class), any());
		return mock;
	}
	...
	static abstract class MetricsProcessorAction implements Answer<Object> {
		...
		public Object answer(InvocationOnMock invocation) throws Throwable {
			@SuppressWarnings("unchecked")
			final MetricProcessor<Object> processor = (MetricProcessor<Object>) invocation
					.getArguments()[0];
		...
		abstract void delegateToProcessor(MetricProcessor<Object> processor,
				MetricName name, Object context) throws Exception;
	}
