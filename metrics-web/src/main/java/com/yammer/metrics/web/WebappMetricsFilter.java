...
public abstract class WebappMetricsFilter implements Filter {
	...
	public WebappMetricsFilter(Map<Integer, String> meterNamesByStatusCode,
		...
		this.metersByStatusCode = new ConcurrentHashMap<Integer, Meter>(
				meterNamesByStatusCode.size());
		for (Entry<Integer, String> entry : meterNamesByStatusCode.entrySet()) {
			metersByStatusCode.put(
					entry.getKey(),
					Metrics.newMeter(WebappMetricsFilter.class,
							entry.getValue(), "responses", TimeUnit.SECONDS));
		}
		this.otherMeter = Metrics.newMeter(WebappMetricsFilter.class,
				otherMetricName, "responses", TimeUnit.SECONDS);
		...
		this.requestTimer = Metrics.newTimer(WebappMetricsFilter.class,
				"requests", TimeUnit.MILLISECONDS, TimeUnit.SECONDS);
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		try {
			...
			requestTimer.update(System.currentTimeMillis() - start,
